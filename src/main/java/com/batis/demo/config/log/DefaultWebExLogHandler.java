...
package com.batis.demo.config.log;

import com.batis.demo.util.ExceptionUtil;
...
public final class DefaultWebExLogHandler implements WebExLogHandler {
    ...
    public void logOnException(HttpServletRequest req, WebExReqAction action, GbpsResponse response, Throwable ex) {
        // logger.
        logger.warn("action={}, 状态码={}, 异常信息：{}", new Object[]{action.getUrl(), response.getStatus(), ExceptionUtil.stackTraceText(ex)});
    }

    @Override
    public void logOnPreAction(HttpServletRequest req, WebExReqAction action) {
        logger.debug("action={} 准备执行，", new Object[]{action.getUrl()});
    }


    @Override
    public void  logOnComplete(HttpServletRequest req, WebExReqAction action, GbpsResponse response) {
        logger.debug("action={} 执行完成，执行时间={}ms，状态码={}", new Object[]{action.getUrl(), action.getRunTimes(), response.getStatus()});
    }
}
